...
if sudo -u ec2-user aws s3 cp s3://silvana-images-testnet/rpc-cert.tar.gz /tmp/rpc-cert.tar.gz 2>/dev/null; then
...
if [ "$cert_from_s3" = false ]; then
    ...
    if sudo test -f "/etc/letsencrypt/live/${DOMAIN_NAME}/fullchain.pem"; then
        ...
        sudo -u ec2-user aws s3 cp rpc-cert.tar.gz s3://silvana-images-testnet/rpc-cert.tar.gz
...
if sudo -u ec2-user aws s3 cp rpc-cert-renewed.tar.gz s3://silvana-images-testnet/rpc-cert.tar.gz; then
