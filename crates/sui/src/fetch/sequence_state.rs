...
use crate::parse::{get_bytes, get_option_bytes, get_option_string, get_u64};
...
pub async fn fetch_sequence_state_by_id(
    ...
    debug!(
        "🔍 Fetching sequence {} from sequence_states table {}",
        sequence, sequence_states_table_id
    );
    ...
    loop {
        ...
        let list_response = client
            ...
            .map_err(|e| {
                SilvanaSuiInterfaceError::RpcConnectionError(format!(
                    "Failed to list sequence states in table: {}",
                    e
                ))
            })?;
        ...
        debug!(
            "📋 Page: found {} dynamic fields (total checked: {})",
            current_page_size, total_fields_checked
        );
        ...
        if page_token.is_none() || current_page_size > 0 {
            ...
            if !found_sequences.is_empty() {
                ...
                if sequence >= min_seq && sequence <= max_seq {
                    debug!(
                        "  Target sequence {} is within page range [{}, {}]",
                        sequence, min_seq, max_seq
                    );
                }
            }
        }
                    ...
                    if let Ok(field_sequence) = bcs::from_bytes::<u64>(name_value) {
                        if field_sequence == sequence {
                            ...
                            if let Some(field_id) = &field.field_id {
                                ...
                                sequence_state_field_request.read_mask =
                                ...
                                let sequence_state_field_response = client
                                    ...
                                    .map_err(|e| {
                                        SilvanaSuiInterfaceError::RpcConnectionError(format!(
                                            "Failed to fetch sequence state field {}: {}",
                                            sequence, e
                                        ))
                                    })?;

                                if let Some(sequence_state_field_object) =
                                    sequence_state_field_response.into_inner().object
                                {
                                                ...
                                                struct_value.fields.get("value")
                                            {
                                                if let Some(
                                                    prost_types::value::Kind::StringValue(
                                                        sequence_state_object_id,
                                                    ),
                                                ) = &value_field.kind
                                                {
                                                    // Fetch the actual sequence state object
                                                    let mut sequence_state_request =
                                                        GetObjectRequest::default();
                                                    sequence_state_request.object_id =
                                                        Some(sequence_state_object_id.clone());
                                                    sequence_state_request.read_mask =
                                                        Some(prost_types::FieldMask {
                                                            paths: vec![
                                                                "object_id".to_string(),
                                                                "json".to_string(),
                                                    ...
                                                    if let Some(sequence_state_object) =
                                                        sequence_state_response.into_inner().object
                                                    {
                                                        if let Some(sequence_state_json) =
                                                            &sequence_state_object.json
                                                        {
                                                            //debug!("🔍 FULL SequenceState JSON for sequence {}: {:#?}", sequence, sequence_state_json);
                                                            if let Ok(sequence_state) =
                                                                extract_sequence_state_from_json(
                                                                    sequence_state_json,
                                                                )
                                                            {
                                                                debug!(
                                                                    "✅ Successfully extracted sequence state {}: has_state={}, has_data_availability={}",
                                                                    sequence,
                                                                    sequence_state.state.is_some(),
                                                                    sequence_state
                                                                        .data_availability
                                                                        .is_some()
                                                                );
                                                                ...
                                                                error!(
                                                                    "❌ Failed to extract sequence state {} from JSON",
                                                                    sequence
                                                                );
                                                            }
                                                        } else {
                                                            error!(
                                                                "❌ No JSON found for sequence state object {}",
                                                                sequence
                                                            );
                                                        }
                                                    } else {
                                                        error!(
                                                            "❌ No sequence state object found for sequence {}",
                                                            sequence
                                                        );
                                                    }
                                                }
        ...
        if let Some(next_token) = response.next_page_token {
            if !next_token.is_empty() {
                debug!(
                    "📄 Moving to next page with token length: {}",
                    next_token.len()
                );
    ...
    debug!(
        "❌ Sequence state {} not found in table {} after checking {} fields",
...
pub async fn get_sequence_state_manager_info_from_app_instance(
    ...
    debug!(
        "Getting SequenceStateManager info from app_instance: {}",
        app_instance.id
    );
    ...
    if app_instance.sequence_state_manager.is_null() {
        debug!(
            "SequenceStateManager not found in app_instance {}",
            app_instance.id
        );
        return Ok(None);
    }
    ...
    if let Some(lowest_value) = manager.get("lowest_sequence") {
        if let Some(seq_str) = lowest_value.as_str() {
            ...
            if let Some(seq_str) = option_obj.as_str() {
                ...
                lowest_sequence = seq_num;
                debug!(
                    "Parsed lowest_sequence from Some as u64: {}",
                    lowest_sequence
                );
            }
        }
    ...
    if let Some(highest_value) = manager.get("highest_sequence") {
        if let Some(seq_str) = highest_value.as_str() {
            ...
            if let Some(seq_str) = option_obj.as_str() {
                ...
                highest_sequence = seq_num;
                debug!(
                    "Parsed highest_sequence from Some as u64: {}",
                    highest_sequence
                );
            }
        }
    ...
    if !table_id.is_empty() {
        debug!(
            "Found SequenceStateManager: lowest={}, highest={}, table_id={}",
            lowest_sequence, highest_sequence, table_id
        );
    ...
    debug!(
        "SequenceStateManager table_id not found in app_instance {}",
        app_instance.id
    );
    Ok(None)
}
...
fn validate_sequence_states(
    ...
    for i in 1..states.len() {
        ...
        if actual != expected {
            ...
            error!(
                ...
                states.len(),
                states[0].sequence,
                states[states.len() - 1].sequence
            );
            return Err(SilvanaSuiInterfaceError::ParseError(format!(
                "Non-sequential sequences: missing sequences between {} and {}",
                states[i - 1].sequence,
                actual
            )));
        }
    ...
    debug!(
        "✅ Sequences are sequential: {} sequences from {} to {}",
        states.len(),
        states[0].sequence,
        states[states.len() - 1].sequence
    );
    ...
    if first_must_have_da && first.data_availability.is_none() && first.sequence != 0 {
        ...
        return Err(SilvanaSuiInterfaceError::ParseError(format!(
            "First sequence {} must have data availability or be sequence 0",
            first.sequence
        )));
    }
    ...
    if first.data_availability.is_some() {
        ...
        debug!(
            "⚠️ First sequence {} has no data availability (allowed in this context)",
            first.sequence
        );
    }
    ...
    if last.sequence != requested_sequence {
        ...
        return Err(SilvanaSuiInterfaceError::ParseError(format!(
            "Last sequence mismatch: got {}, expected {}",
            last.sequence, requested_sequence
        )));
    }
    debug!(
        "✅ Last sequence {} matches requested sequence",
        last.sequence
    );
...
pub async fn query_sequence_states(
    ...
    debug!(
        ...
        app_instance.id, requested_sequence
    );

    // Get SequenceStateManager info from AppInstance
    let (lowest_sequence, highest_sequence, table_id) =
        match get_sequence_state_manager_info_from_app_instance(app_instance).await? {
            Some(info) => {
                debug!(
                    "SequenceStateManager found: lowest={}, highest={}, table_id={}",
    ...
    if requested_sequence < lowest_sequence || requested_sequence > highest_sequence {
        error!(
            "Requested sequence {} is out of bounds [{}, {}]",
    ...
    debug!(
        "Requested sequence {} is within bounds [{}, {}], fetching state",
        requested_sequence, lowest_sequence, highest_sequence
    );
    ...
    let requested_state = match fetch_sequence_state_by_id(&table_id, requested_sequence).await? {
        Some(state) => {
            debug!(
                "Successfully fetched requested sequence state {}: has_state={}, has_data_availability={}",
                requested_sequence,
                state.state.is_some(),
        ...
        None => {
            error!(
                "Sequence state {} not found in table {}",
                requested_sequence, table_id
            );
            return Err(SilvanaSuiInterfaceError::ParseError(format!(
                "Sequence state {} not found",
                requested_sequence
            )));
        }
    };
    ...
    if requested_state.data_availability.is_some() {
        ...
        debug!(
            "Requested sequence {} has data availability, returning single state",
            requested_sequence
        );
    ...
    debug!(
        "Requested sequence {} does not have data availability, searching for highest sequence with DA",
        requested_sequence
    );
    ...
    for seq in (lowest_sequence..requested_sequence).rev() {
        ...
        if let Ok(Some(state)) = fetch_sequence_state_by_id(&table_id, seq).await {
            debug!(
                "Sequence {}: has_state={}, has_data_availability={}",
                seq,
                state.state.is_some(),
                state.data_availability.is_some()
            );
    ...
    match start_sequence {
        Some(seq) => {
            ...
            debug!(
                "Found start sequence {} with data availability, fetching all sequences from {} to {}",
            ...
            for sequence_num in seq..=requested_sequence {
                if let Ok(Some(state)) = fetch_sequence_state_by_id(&table_id, sequence_num).await {
                    ...
                    debug!(
                        "Warning: Could not fetch sequence state for {}",
                        sequence_num
                    );
                }
            }
            ...
            debug!(
                "Fetched {} sequence states, validating before return",
                result_states.len()
            );
            ...
            debug!(
                "✅ Validation passed, returning {} sequence states from {} to {}",
                result_states.len(),
                seq,
                requested_sequence
            );
        ...
        None => {
            ...
            debug!(
                "No sequence with data availability found, returning all sequences from {} to {}",
