use anyhow::Result;
use sui::publish::{build_move_package, build_move_package_ignore_chain};
...
async fn test_build_move_package_with_dependencies() -> Result<()> {
    ...
    if !path.exists() {
        eprintln!("Skipping test: example package not found at {}", path.display());
        return Ok(());
    }
    ...
    assert_eq!(result.modules.len(), 1, "The 'add' package should have exactly one module");
    ...
    assert!(result.dependencies.len() >= 3, "Should have at least 3 dependencies (Sui, MoveStdlib, and local deps)");
    
    // Verify digest
    assert_eq!(result.digest.len(), 32, "Digest should be 32 bytes (SHA256)");
    ...
    assert!(
        first_module.chars().all(|c| c.is_ascii_alphanumeric() || c == '+' || c == '/' || c == '='),
...
async fn test_build_nonexistent_package() -> Result<()> {
    ...
    let result = build_move_package_ignore_chain("/path/that/does/not/exist").await;
...
async fn test_build_output_structure() -> Result<()> {
    ...
    for dep in &result.dependencies {
        assert!(dep.starts_with("0x"), "Each dependency should be a hex address");
    ...
    let digest_hex: String = result.digest
        .iter()
        .map(|b| format!("{:02x}", b))
        .collect();
    ...
    for (i, dep) in result.dependencies.iter().enumerate() {
        ...
        if dep.starts_with("0x") {
            ...
            match STANDARD.decode(dep) {
                Ok(bytes) => {
                    println!("Dependency {} decoded successfully: {} bytes", i, bytes.len());
                }
                Err(_) => {
