...
use crate::{
    ...
    status::{get_operator_status, get_operator_details},
};

#[derive(Parser)]
#[command(
    name = "avs",
    about = "AVS Operator commands for managing EigenLayer AVS operations",
    version,
    author
)]
pub struct AvsCliArgs {
    #[command(subcommand)]
    pub command: AvsCommands,
}
...
/// Execute AVS command
pub async fn execute_avs_command(command: AvsCommands) -> Result<()> {
    ...
    // Execute command
    match command {
        AvsCommands::Register { chain_id, rpc_url } => {
        ...
        AvsCommands::Monitor { polling, response_percentage } => {
        ...
        AvsCommands::Task { name, use_aggregator } => {
        ...
        AvsCommands::Status { address, detailed } => {
            if detailed {
                info!("Fetching detailed operator status...");
                get_operator_details(&config, address).await?;
            } else {
