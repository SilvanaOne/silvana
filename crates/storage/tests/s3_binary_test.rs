...
async fn test_write_and_read_binary() -> Result<()> {
    ...
    let stored_hash = s3_client
        .write_binary(data.clone(), &file_name, "application/octet-stream", None)
        .await?;
    ...
    assert_eq!(read_data, data, "Read data should match original data");
    assert_eq!(read_hash, expected_hash, "Read hash should match expected hash");
    ...
    let large_hash = s3_client
        .write_binary(large_data.clone(), &file_name_large, "application/octet-stream", None)
        .await?;
    
    let (read_large_data, read_large_hash) = s3_client.read_binary(&file_name_large).await?;
    assert_eq!(read_large_data.len(), large_data.len(), "Large data size should match");
    assert_eq!(read_large_hash, large_hash, "Large data hash should match");
...
async fn test_binary_with_different_mime_types() -> Result<()> {
    ...
    for (mime_type, data, suffix) in test_cases {
        ...
        let (read_data, read_hash) = s3_client.read_binary(&file_name).await?;
        assert_eq!(read_data, data, "Data should match for MIME type {}", mime_type);
        assert_eq!(read_hash, hash, "Hash should match for MIME type {}", mime_type);
...
async fn test_binary_edge_cases() -> Result<()> {
    ...
    let empty_hash = s3_client
        .write_binary(empty_data.clone(), &file_name_empty, "application/octet-stream", None)
        .await?;
    
    let (read_empty, read_empty_hash) = s3_client.read_binary(&file_name_empty).await?;
    assert_eq!(read_empty.len(), 0, "Empty file should have 0 bytes");
    assert_eq!(read_empty_hash, empty_hash, "Empty file hash should match");
    ...
    let special_hash = s3_client
        .write_binary(special_data.clone(), &file_name_special, "application/octet-stream", None)
        .await?;
    
    let (read_special, read_special_hash) = s3_client.read_binary(&file_name_special).await?;
    assert_eq!(read_special, special_data, "Special bytes should be preserved");
    assert_eq!(read_special_hash, special_hash, "Special bytes hash should match");
    ...
    let complex_hash = s3_client
        .write_binary(complex_data.clone(), &file_name_complex, "application/octet-stream", None)
        .await?;
    
    let (read_complex, read_complex_hash) = s3_client.read_binary(&file_name_complex).await?;
    assert_eq!(read_complex, complex_data, "Complex filename data should match");
    assert_eq!(read_complex_hash, complex_hash, "Complex filename hash should match");
