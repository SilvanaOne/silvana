...
async fn fetch_dynamic_fields(parent_id: &str) -> Result<Value> {
    ...
    loop {
        ...
        // Debug: Log the first field to see what's being returned
        if !fields_response.dynamic_fields.is_empty() && pages_fetched == 1 {
            let first_field = &fields_response.dynamic_fields[0];
            debug!("First dynamic field structure: {:?}", first_field);
            debug!("Field has name: {}", first_field.name.is_some());
            debug!("Field has field_id: {}", first_field.field_id.is_some());
            debug!("Field has value_type: {}", first_field.value_type.is_some());
            debug!("Field has parent: {}", first_field.parent.is_some());
            debug!("Field has child_id: {}", first_field.child_id.is_some());
        }
        ...
        for field in fields_response.dynamic_fields {
            ...
            // Try to decode name_value from name field if available
