...
use crate::transactions::{execute_transaction_block, PublishOptions};
...
async fn extract_package_id_from_digest(digest: &str) -> Result<String> {
    ...
    if let Some(executed_tx) = &response.transaction {
        ...
        if let Some(effects) = &executed_tx.effects {
            ...
            for changed_obj in &effects.changed_objects {
                ...
                if let (Some(input_state), Some(output_state)) = (changed_obj.input_state, changed_obj.output_state) {
                        ...
                        if let Some(output_owner) = &changed_obj.output_owner {
                            ...
                            if let Some(kind) = output_owner.kind {
                                if kind == owner::OwnerKind::Immutable as i32 {
                                    if let Some(object_id) = &changed_obj.object_id {
                                        info!("Found published package ID (immutable object): {}", object_id);
                                        return Ok(object_id.clone());
                                    }
                                }
                            }
    ...
    Err(anyhow!("Could not find published package ID in transaction effects"))
}
...
pub async fn publish_move_package(
    ...
    info!("Publishing Move package with {} modules", build_result.modules.len());
    ...
    let digest = execute_transaction_block::<fn(&mut sui_transaction_builder::TransactionBuilder, Vec<sui_sdk_types::Argument>, sui_sdk_types::Argument) -> Vec<sui_sdk_types::Argument>>(
        package_id,
...
pub async fn build_and_publish_move_package(
    ...
    // Build the package
    let build_result = build_move_package_ignore_chain(path).await?;
